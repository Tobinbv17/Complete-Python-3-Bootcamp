...
atac_tracks <- lapply(1:nrow(track_mapping), function(i) {
  ...
  track_label <- paste0(row$genotype, '\n', row$timepoint, ' Il6 stim')
  
  DataTrack(
    range = full_file_path,
    genome = 'mm10', 
    name = track_label, 
    type = 'mountain',   # 'mountain' draws a clean flat base resembling smooth blocks 
    fill.mountain = c(track_color, track_color),
    col.mountain = "transparent", # Removes jagged border lines for a cleaner block appearance
    ...
    fontface.title = "bold",
    cex.title = 0.8,
    rot.title = 0          # FIXED POINT 1: Set to 0 so names print horizontally on single lines!
  )
})
...
# FIXED POINT 2: Defined the correct STAT3 binding coordinates mapped in genomic space
fimo_ranges = GRanges(
  seqnames = target_chr, 
  ranges = IRanges(
...
fimoTrack = AnnotationTrack(
  ...
  cex.title = 0.8,
  col.title = "black",
  rot.title = 0     # FIXED POINT 1: Keep labels horizontal
)
...
names(geneTrack) <- "Dnmt3a isoforms"
displayPars(geneTrack) <- list(
  fill = "#0d47a1", # Darker blue matching paper's transcripts
  col = "#0d47a1",
  showId = TRUE,             # FIXED POINT 3: Show individual transcript variant names
  collapseTranscripts = FALSE, # FIXED POINT 3: Force Gviz to draw all 5-6 isoform lines separately
  geneSymbol = TRUE,
  fontface.title = "bold",
  cex.title = 0.8,
  col.title = "black",
...
track_sizes = c(0.5, rep(1, length(atac_tracks)), 0.6, 2.0) # Expanded height for all transcripts

png('~/project_analysis/PTPN2i/20260706_PTPN2i_scRNA_paper_swyz/Panel_D_Dnmt31_Fixed.png', 
    width = 1600, height = 1100, res = 150) # Added a bit more height to fit extra isoforms gracefully
...
plotTracks(
  ...
  col.title = 'black', 
  col.axis = 'black',
  # Ensures plenty of margin room so text is completely visible on one horizontal line
  title.width = 4.0  
)
